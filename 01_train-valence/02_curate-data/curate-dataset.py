...
def download_and_filter_core_td_data(
    td_datasets: typing.List[str],
    td_records_to_remove: typing.Optional[str] = None,
    only_download_these_records_core: typing.Optional[str] = None,
    include_iodine: bool = False,
) -> "TorsionDriveResultCollection":
    """Download and filter core torsiondrive datasets without whitelist filtering."""
    ...
    if td_records_to_remove is not None:
        ...
        # print(f"Number of records to remove: {len(records_to_remove)}")
    else:
        records_to_remove = []

    #Load whitelist
    if only_download_these_records_core is not None:
        records_to_keep = np.loadtxt(only_download_these_records_core, dtype=str)
        # print(f"Number of records to keep is: {len(records_to_keep)}")
    else:
    ...
    print(f"Number of core TD records after download: {len(dataset.entries[client.address])}")
    
    # #for entry in dataset.entries[client.address][:5]:
    # #    print("Sample record ID:", entry.record_id, type(entry.record_id))
    # if records_to_keep is not None:
    #     print("Entries in records_to_keep:")
    #     for entry in records_to_keep:
    #         print(entry)
    # else:
    #     print("No entries in records_to_keep")

    # apply whitelist first (only keep specific IDs)
    if records_to_keep is not None:
        records_to_keep_set = set(str(r).strip() for r in records_to_keep)
        dataset.entries[client.address] = [
            entry
            for entry in dataset.entries[client.address]
            if str(entry.record_id).strip() in records_to_keep_set
        ]
    # print(f"After whitelist filter: {dataset.n_results}")
    # Filter out records to remove (blacklist)
    if len(records_to_remove) > 0:
        dataset.entries[client.address] = [
            entry for entry in dataset.entries[client.address]
            if str(entry.record_id) not in records_to_remove
        ]
    # print(f"Number of core TD records after removing blacklist: {len(dataset.entries[client.address])}")
...
def download_and_filter_aux_td_data(
    td_datasets: typing.List[str],
    td_records_to_remove: typing.Optional[str] = None,
    only_download_these_records_aux: typing.Optional[str] = None, 
    include_iodine: bool = False,
) -> "TorsionDriveResultCollection":
    ...
    # Load blacklist (records to remove)
    if td_records_to_remove is not None:
        records_to_remove = np.loadtxt(td_records_to_remove, dtype=str)
        # print(f"Number of records to remove: {len(records_to_remove)}")
    else:
        records_to_remove = []
        # print("No td_records_to_remove file provided; skipping blacklist filter.")

    # Load the whitelist (records to keep)
    if only_download_these_records_aux is not None:
        records_to_keep = np.loadtxt(only_download_these_records_aux, dtype=str)
        # print(f"Number of records to keep is: {len(records_to_keep)}")
    else:
        records_to_keep = None
        # print("No records to keep; downloading all aux records")
    ...
    # print(f"Number of aux TD records after download: {dataset.n_results}")
    
    #print("Dataset record IDs (raw):")
    #for entry in dataset.entries[client.address][:10]:  # print first 10 for brevity
    #    print(entry.record_id)

    #print("Dataset record IDs (processed):")
    #for entry in dataset.entries[client.address][:10]:
    #    print(str(entry.record_id).replace("torsion-", ""))
    ...
    #         print(entry)
    # else:
    #     print("No entries in records_to_keep (whitelist not used)")

    # Apply whitelist filter first (only keep specified IDs)
    if records_to_keep is not None:
        records_to_keep_set = set(str(r).strip() for r in records_to_keep)
        dataset.entries[client.address] = [
            entry
    ...
    # print(f"After whitelist filter: {dataset.n_results}")
    # Apply blacklist filter second (remove unwanted IDs)
    if len(records_to_remove) > 0:
        dataset.entries[client.address] = [
            entry
            for entry in dataset.entries[client.address]
    ...
    # print(f"After backlist filter: {dataset.n_results}")
...
@click.option(
    "--only-download-these-records-core",
    type=click.Path(exists=True, dir_okay=False, file_okay=True),
    default=None,
    help=(
        "Path to a file containing a list of torsiondrive record IDs to include. "
        "Only these records will be retained, all others excluded."
    ),
)
@click.option(
    "--only-download-these-records-aux",
    type=click.Path(exists=True, dir_okay=False, file_okay=True),
    default=None,
    help=(
        "Path to a file containing a list of torsiondrive record IDs to include from aux dataset only. "
        "Only these records will be retained, all others excluded."
    ),
)
@click.option(
...
def download_td_data(
    ...
    only_download_these_records_core: typing.Optional[str] = None,  # added
    only_download_these_records_aux: typing.Optional[str] = None,
    ...
    core_dataset = download_and_filter_core_td_data(
        core_td_datasets, td_records_to_remove, only_download_these_records_core, include_iodine=False
    )
    if verbose:
        print(f"Number of core entries: {core_dataset.n_results}")

    # # download and filter auxilliary dataset(s)
    # aux_dataset = download_and_filter_aux_td_data(
    #     aux_td_datasets, td_records_to_remove, only_download_these_records_aux, include_iodine=False,
    # )
    # if verbose:
    #     print(f"Number of aux entries: {aux_dataset.n_results}")
    ...
    # all_entries = cap_torsions_per_parameter(
    #     ff,
    #     all_entries,
    #     cap_size=cap_size,
    #     method=cap_method,
    ...
    # )
    # if verbose:
    #     print(f"Number of entries after capping: {all_entries.n_results}")

    # filtered_for_charge = core_dataset.filter(ChargeCheckFilter())
    ...
    # the script below used to be a filtering block at the end, but i put an aux filtering block and a core filtering block instead 
#    if only_download_these_records is not None:
#        print("Filtering aux and core with 'only_download_these_records' entry")
#        with open(only_download_these_records, "r") as f:
#            allowed_ids = set(line.strip() for line in f if line.strip())
#        unique_entries = {
#            record_id: record
#            for record_id, record in unique_entries.items()
#            if str(record_id) in allowed_ids
#        }
#        if verbose:
#            print(f"Filtered to only {len(unique_entries)} user-specified torsion records.")

    # new_dataset = TorsionDriveResultCollection(
    #     entries={key: list(unique_entries.values())}
    # )

    #report the number of records per parameter
    coverage, parameter_records = get_parameter_distribution(
    ...
    print("Final coverage from main code")
    for parameter_id, records in records_to_keep.items():
        print(
            f"{parameter_id:>6s}: {len(records):>4d} "
            f"/ {coverage[parameter_id]:>4d} records"
        )
    #save this to a dictionary with {parameter_id}
    ...
    for parameter_id, records in records_to_keep.items():
        parameter_counts[parameter_id] = len(records)
    
    with open(output_count_file, "w") as f:
        json.dump(parameter_counts, f, indent=2)
...
@click.option(
    "--only-download-these-records",
    type=click.Path(exists=True, dir_okay=False, file_okay=True),
    default=None,
    help="The path to a file with the records to include. Notice that only the records in this list will be included" \
    "All others will be excluded. This is a way to filter only 1 record for simplification",
)
@click.option(
    "--verbose",
    is_flag=True,
...
def download_opt_data(
    ...
    opt_records_to_remove: typing.Optional[str] = None,
    only_download_these_records: typing.Optional[str] = None,
    max_opt_conformers: int = 12,
    verbose: bool = True,
    n_processes: int = 1,
    min_record_coverage: int = 5,
    output_count_file: str = "optgeo-counts.json",
):
    """Download and filter optimization datasets.
    ...
    # optionally filter to only include specific record IDs
    if only_download_these_records is not None:
        with open(only_download_these_records, "r") as f:
            allowed_ids = set(line.strip() for line in f if line.strip())
        unique_entries = {
        ...
        if verbose:
            print(f"Filtered to only {len(unique_entries)} user-specified records.")
...
@cli.command("generate-smirks")
@click.option(
    "--input",
    "input_path",
    required=True,
    type=click.Path(exists=True, dir_okay=False, file_okay=True),
    help="The path to an existing filtered optimization dataset JSON."
)
@click.option(
    "--output-parameter-smirks",
    "output_parameter_smirks_path",
    required=True,
    type=click.Path(exists=False, dir_okay=False, file_okay=True),
    help="The path to write the parameter SMIRKS JSON."
)
@click.option(
    "--initial-forcefield",
    required=True,
    type=str,
    help="The path to the initial force field file."
)
@click.option(
    "--min-record-coverage",
    type=int,
    default=5,
    show_default=True,
    help="Minimum number of records required to include a parameter."
)
@click.option(
    "--output-count-file",
    "-oc",
...
@click.option(
    ...
    default=None,
    help=(
        "The path to a file containing optimization record IDs to keep from "
        "the input dataset."
    ),
)
@click.option(
    ...
    type=click.Path(exists=False, dir_okay=False, file_okay=True),
    default=None,
    help="The path to write the filtered optimization dataset JSON.",
)
@click.option(
    "--verbose",
    is_flag=True,
...
def generate_smirks(
    ...
    only_download_these_records: typing.Optional[str] = None,
    output_filtered_dataset: typing.Optional[str] = None,
):
    from openff.qcsubmit.results import OptimizationResultCollection
    from openff.toolkit import ForceField
    import json
    ...
    dataset = OptimizationResultCollection.parse_file(input_path)

    if only_download_these_records is not None:
        with open(only_download_these_records, "r") as file:
            allowed_ids = {line.strip() for line in file if line.strip()}

        kept_ids = set()
        filtered_entries = {}
        for address, entries in dataset.entries.items():
            kept_entries = [
                entry for entry in entries if str(entry.record_id) in allowed_ids
            ]
            if kept_entries:
                filtered_entries[address] = kept_entries
                kept_ids.update(str(entry.record_id) for entry in kept_entries)

        dataset = OptimizationResultCollection(entries=filtered_entries)

        if verbose:
            print(f"Filtered to {dataset.n_results} user-specified records.")
            missing_ids = sorted(allowed_ids - kept_ids, key=lambda value: int(value))
            if missing_ids:
                print(
                    "Warning: these requested records were not found in the input "
                    f"dataset: {', '.join(missing_ids)}"
                )

    if output_filtered_dataset is not None:
        with open(output_filtered_dataset, "w") as file:
            file.write(dataset.json(indent=2))
        if verbose:
            print(f"Saved filtered dataset to {output_filtered_dataset}")
    ...
    # Compute coverage counts per parameter
    coverage, parameter_records = get_parameter_distribution(
        dataset=dataset,
        parameter_types=parameter_types,
        force_field=ff,
    ...
    parameter_counts: dict[str, int] = {k: 0 for k in all_parameter_ids}
    for parameter_id, records in parameter_records.items():
        parameter_counts[parameter_id] = len(records)
    ...
    output_data = {
        "smirks": selected_parameters,
        "counts": parameter_counts
    }

    with open(output_parameter_smirks_path, "w") as f:
        json.dump(output_data, f, indent=2)

    if verbose:
        print(f"Wrote parameter SMIRKS and counts to {output_parameter_smirks_path}")
